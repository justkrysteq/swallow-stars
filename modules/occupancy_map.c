...
// NOTE: I know that type and entity can be accessed via prev_y and prev_x but it requires a change in implementation while spawning
void update_occupancy_map(OCCUPANT **occupancy_map, const unsigned int prev_y, const unsigned int prev_x, const unsigned int y, const unsigned int x, char type, void *entity, STATE *game_state) {
	if (x != prev_x || y != prev_y) {
		// char type = occupancy_map[prev_y][prev_x].type;
		// void *entity = occupancy_map[prev_y][prev_x].entity;
